...
gc.enable() # BIKIN GC NYALA TERUS
...
def area_aktif(area, positions): 
    return any(p['area'] == area for p in positions)
        ...
        if s['symbol'] == PAIR:
            for f in s['filters']:
                if f['filterType']=='MIN_NOTIONAL': 
                    min_n_binance = float(f['minNotional'])
...
def binance_order_market(side, qty):
    params = {"side":side, "type":"MARKET", "quantity":qty}
...
async def safe_send(chat_id, msg):
    try:
        ...
        pass # ANTI CRASH TELEGRAM

async def notif_event(msg):
    if TELE_CHAT_ID: 
        await safe_send(TELE_CHAT_ID, msg)
...
async def sinkron_db_dengan_binance():
    ...
    del positions_db # HAPUS DARI RAM
    gc.collect()
...
async def satpam_buy(price, area, reason="GRID"):
    ...
    try:
        ...
        order_status = binance_get_order(order['orderId'])
...
async def satpam_sell_area(area, positions_in_area, price, mode="BIASA"):
    ...
    try:
        ...
        is_executing = False
        del positions_in_area # HAPUS DARI RAM
        gc.collect()
...
async def satpam_sell_instansemua(all_positions, price):
    ...
    try:
        ...
        is_executing = False
        del all_positions # HAPUS DARI RAM
        gc.collect()
...
async def scout_loop(context: ContextTypes.DEFAULT_TYPE):
    ...
    positions = get_positions() # AMBIL DARI SUPABASE
    ...
    del positions # HAPUS DARI RAM SETELAH SELESAI CEK
    gc.collect()
...
async def main():
    ...
    app = ApplicationBuilder().token(TELE_TOKEN).pool_timeout(10.0).build() # TAMBAH POOL_TIMEOUT
    ...
    await cek_pengaman_restart(base_price_start, db) # DIPAKE DULU
    del db # BARU HAPUS
    gc.collect()
