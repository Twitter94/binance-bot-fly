...
SUPABASE_KEY = os.getenv("SUPA_KEY") # HARUS service_role key
...
FLAG_FILE = "/tmp/table_created.flag" # <--- PENANDA 1X SEUMUR HIDUP
...
# ========== AUTO CREATE TABLE 1X ==========
def auto_create_table():
    if os.path.exists(FLAG_FILE):
        return # Udah pernah bikin, skip biar gak spam
    ...
    drop policy if exists "Allow all" on public.{TABEL};
    create policy "Allow all" on public.{TABEL} for all to public using (true) with check (true);
    """
    try:
        r = requests.post(f"{SUPABASE_URL}/pg", headers=SB_HEADERS, json={"query": sql}, timeout=10)
        if r.status_code in [200, 201, 204]:
            send_telegram("✅ Tabel `orders` + RLS auto dibuat 1x")
            with open(FLAG_FILE, "w") as f: f.write("done") # Kasih tanda
        else:
            send_telegram(f"⚠️ GAGAL BUAT TABEL. Cek SUPA_KEY harus service_role\nErr: {r.status_code}")
...
def signed_request(method, endpoint, params={}):
    try:
        ...
        params['recvWindow'] = 60000 # Anti timeout 400
...
def format_qty(qty): # FIX ANTI 6e-05
...
async def main():
    ...
    auto_create_table() # Cuma jalan 1x
    get_binance_rules(SYMBOL)
    send_telegram("🤖 <b>Bot V11.60 START</b> ANTI SPAM ON")
