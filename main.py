...
# ========== INI FUNGSI BARU: BIKIN TABEL OTOMATIS ==========
def buat_tabel_otomatis():
    """Bikin tabel orders kalau belum ada. Jalan 1x pas bot start"""
    sql = f"""
    ...
    create policy "Allow all" on public.{TABEL} for all using (true) with check (true);
    """
    try:
        # Cara 1: Pakai RPC kalau ada
        r = requests.post(f"{SUPABASE_URL}/rest/v1/rpc/query", headers=SB_HEADERS, json={"query": sql}, timeout=10)
        if r.status_code not in [200, 201]:
            # Cara 2: Insert dummy biar tabel kebuat
            requests.post(f"{SUPABASE_URL}/rest/v1/{TABEL}", headers=SB_HEADERS, json={"price":0,"qty":0,"side":"INIT","status":"INIT"}, timeout=5)
            requests.delete(f"{SUPABASE_URL}/rest/v1/{TABEL}?price=eq.0", headers=SB_HEADERS, timeout=5)
        send_telegram("✅ Tabel 'orders' siap")
    except Exception as e:
        send_telegram(f"⚠️ Gagal buat tabel: {e}")
...
# ========== INI YANG DIBENERIN TOTAL ANTI CRASH ==========
def sb_get_all():
    try:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/{TABEL}?select=price", headers=SB_HEADERS, timeout=5)
        if r.status_code!= 200: # Kalau 404 tabel belum ada, 401 key salah
            return []
        data = r.json()
        # FILTER KERAS: harus list, harus dict, harus ada 'price'
        return [x for x in data if isinstance(x, dict) and 'price' in x]
    except:
        return []

#... SISA CODE KAMU TETEP SAMA DARI signed_request SAMPAI BAWAH...
...
def hitung_butuh_modal(price, qty, buffer=BUFFER_USDT):
    ...
    fee_total = modal_dasar * (FEE_BINANCE * 2)
...
async def main():
    ...
    buat_tabel_otomatis() # <-- PANGGIL INI PERTAMA KALI
    ...
    send_telegram(f"🤖 <b>Bot V11.47.0 AUTO-TABEL</b>\n<b>Harga BTC:</b> {harga_sekarang:.2f}\n<b>Saldo:</b>\nUSDT: {saldo_usdt:.2f}\nBTC: {saldo_btc:.6f}")
    while True:
        try:
