...
def auto_create_table():
    """Nembak SQL ke Supabase buat bikin tabel + RLS kalau belum ada"""
    sql = f"""
    ...
    """
    try:
        # Pake endpoint pg buat jalanin raw SQL
        r = requests.post(
            f"{SUPABASE_URL}/pg",
            headers=SB_HEADERS,
            json={"query": sql},
        ...
        if r.status_code in [200, 201, 204]:
            send_telegram("✅ Tabel `orders` + RLS auto dibuat/cek OK")
        else:
            send_telegram(f"⚠️ Gagal auto create tabel: {r.status_code} {r.text}")
...
def format_qty(qty): # FIX FINAL ANTI 6e-05
...
async def main():
    ...
    auto_create_table() # AUTO BIKIN TABEL DI SINI
    get_binance_rules(SYMBOL)
    send_telegram("🤖 <b>Bot V11.58 START</b> AUTO TABLE ON")
