...
SYMBOL = "BTCUSDT"; LOOP_SEC = 3; BUFFER_USDT = 0.5; TABEL = "orders"
RECOVERY_INTERVAL = 3600; RE_ENTRY_MODE = True; REENTRY_COOLDOWN = 60
ATR_PERIOD = 14; ATR_TIMEFRAME = "1h"; ATR_MULTIPLIER = 0.5; ATR_UPDATE_HOUR = 0
MIN_JARAK = 250; MAX_JARAK = 1000; JSON_FILE = "pending_orders.json"
WAIT_FIRST_BUY = 60; FIRST_BUY_DONE = False; START_TIME = time.time()
LAST_RECOVERY = 0; BUYING_LOCK = set(); PERLU_REENTRY = False; LAST_REENTRY_TIME = 0
...
def cek_command_telegram():
    try:
        url = f"https://api.telegram.org/bot{TELE_TOKEN}/getUpdates"; r = requests.get(url, timeout=3).json()
        if 'result' not in r or len(r['result']) == 0: return
        last_update = r['result'][-1];
        if 'message' not in last_update: return
        text = last_update['message'].get('text', '').lower(); chat_id = str(last_update['message']['chat']['id'])
        if chat_id!= TELE_CHAT_ID: return
        global NOTIF_MODE
        if text == "ganti mode": NOTIF_MODE = "NORMAL" if NOTIF_MODE == "SILENT" else "SILENT"; txt = "🔊 MODE NORMAL AKTIF" if NOTIF_MODE == "NORMAL" else "🔇 MODE SILENT AKTIF"; notif_penting(txt); requests.get(f"https://api.telegram.org/bot{TELE_TOKEN}/getUpdates?offset={last_update['update_id']+1}")
...
def load_and_clear_json():
    ...
    try:
        with open(JSON_FILE, 'r') as f: pending = json.load(f)
        os.remove(JSON_FILE); return pending
    except: return []
...
def cek_tabel_supabase():
    try:
        ...
        if r.status_code == 200:
            ...
            if len(pending) > 0: notif_penting(f"🔄 Menemukan {len(pending)} order di JSON. Mencoba insert ke DB..."); [sb_insert(p) for p in pending]
...
def sb_insert(data):
    try: r = requests.post(f"{SUPABASE_URL}/rest/v1/{TABEL}", headers=SB_HEADERS, json=data, timeout=5); return r.json() if r.status_code in [200,201] else []
    except: return []

def sb_select(filters=""):
    try: r = requests.get(f"{SUPABASE_URL}/rest/v1/{TABEL}?{filters}", headers=SB_HEADERS, timeout=5); data = r.json(); return data if isinstance(data, list) else []
    except: return []
...
def bersihin_sampah():
    ...
    try: requests.delete(f"{SUPABASE_URL}/rest/v1/{TABEL}?status=neq.OPEN&time=lt.{tujuh_hari_lalu}", headers=SB_HEADERS, timeout=5); log_only("🧹 Sampah DB CLOSED>7hari dihapus")
    except: pass
    global NOTIF_FLAGS; NOTIF_FLAGS = {"error": False, "saldo_kurang": False, "critical_msg": NOTIF_FLAGS["critical_msg"]}; gc.collect()

def sync_3_sumber():
    global PERLU_REENTRY; count = 0
    data_binance = signed_request("GET", "/api/v3/allOrders", {"symbol":SYMBOL, "limit": 500})
    data_db = sb_select(f"status=eq.OPEN"); data_json = load_and_clear_json()
    if not isinstance(data_binance, list): data_binance = []
    if len(data_json) > 0: [sb_insert(p) for p in data_json]; data_db = sb_select(f"status=eq.OPEN")
...
def signed_request(method, endpoint, params=None):
    ...
    try:
        ...
        query_string = urlencode(params); signature = hmac.new(BINANCE_SECRET.encode(), query_string.encode(), hashlib.sha256).hexdigest()
        url = f"{BASE_URL}{endpoint}?{query_string}&signature={signature}"; headers = {'X-MBX-APIKEY': BINANCE_API_KEY}
        r = requests.request(method, url, headers=headers, timeout=10); return r.json() if r.status_code == 200 else {}
    except: return {}
...
def get_all_balance():
    ...
    usdt = float(next((b['free'] for b in data['balances'] if b['asset']=='USDT'), 0)); btc = float(next((b['free'] for b in data['balances'] if b['asset']=='BTC'), 0))
    return usdt, btc

def get_binance_rules(symbol):
    try: data = requests.get(f"{BASE_URL}/api/v3/exchangeInfo?symbol={symbol}", timeout=5).json(); [setattr(BINANCE_RULES, 'min_notional', float(f['minNotional'])) if f['filterType']=='MIN_NOTIONAL' else None or setattr(BINANCE_RULES, 'min_qty', float(f['minQty'])) or setattr(BINANCE_RULES, 'step_size', float(f['stepSize'])) if f['filterType']=='LOT_SIZE' else None for f in data['symbols'][0]['filters']]
    except: pass

def get_binance_fee():
    try: data = signed_request("GET", "/api/v3/account"); return float(data['takerCommission']) / 10000 if 'takerCommission' in data else 0.001
    except: return 0.001

def format_qty(qty): step = BINANCE_RULES['step_size']; min_qty = BINANCE_RULES['min_qty']; qty_floored = int(qty / step) * step; return f"{min_qty if qty_floored < min_qty else qty_floored:.8f}"

def hitung_qty_aman(harga): qty = max(BINANCE_RULES['min_qty'], BINANCE_RULES['min_notional'] / harga); qty_formatted = float(format_qty(qty)); return format_qty(qty_formatted + BINANCE_RULES['step_size'] if harga * qty_formatted < BINANCE_RULES['min_notional'] else qty_formatted)
...
def get_atr(symbol, period=ATR_PERIOD, interval=ATR_TIMEFRAME):
    try: data = requests.get(f"{BASE_URL}/api/v3/klines?symbol={symbol}&interval={interval}&limit={period+1}", timeout=10).json(); tr_list = [max(float(data[i][2])-float(data[i][3]), abs(float(data[i][2])-float(data[i-1][4])), abs(float(data[i][3])-float(data[i-1][4]))) for i in range(1, len(data))]; return sum(tr_list[-period:]) / period
    except: return 0
...
def is_price_exist(price): jarak = ATR_MANAGER["jarak"] if ATR_MANAGER["jarak"] else MIN_JARAK; return len(sb_select(f"price=gte.{price-jarak/2}&price=lte.{price+jarak/2}&side=eq.BUY&status=eq.OPEN")) > 0
...
def cek_signal_sell(price):
    ...
    if len(data_open) > 0 and price >= data_open[0]['price'] + ATR_MANAGER["jarak"]:
        data_tertinggi = sb_select(f"status=eq.OPEN&side=eq.BUY&order=price.desc&limit=1"); return True, price, data_open[0], len(data_tertinggi) > 0 and data_tertinggi[0]['id'] == data_open[0]['id']
...
def cek_sell_instan_darurat(price):
    ...
    if len(data_db) > 0:
        ...
        if harga_buy_pertama > 0 and price > harga_buy_pertama:
            ...
            if nilai_jual >= butuh_min: res = signed_request("POST", "/api/v3/order", {"symbol":SYMBOL, "side":"SELL", "type":"MARKET", "quantity":qty}); [sb_delete(d['id']) for d in data_db] if 'orderId' in res else None; notif_penting(f"✅ MODE 1 SUKSES\nJual {qty} @ {price:.2f}\nProfit Kotor: {(price - harga_buy_pertama) * float(qty):.4f} USDT") if 'orderId' in res else None

def recovery_sync(): sync_3_sumber()

def cek_order_binance_sudah_ada(price_target): data = signed_request("GET", "/api/v3/openOrders", {"symbol":SYMBOL}); return any(abs(float(o['price']) - price_target) < 0.01 for o in data) if isinstance(data, list) else False
...
def place_order_real(side, price_grid, qty, order_data=None, is_top_grid=False):
        ...
        try:
            ...
            if 'orderId' in res:
                ...
                if len(sb_select(f"binance_order_id=eq.{order_id}")) == 0:
                    if len(sb_insert({"price":price_grid, "qty":float(qty), "side":"BUY", "status":"OPEN", "binance_order_id": order_id, "fee": fee_buy, "time": int(time.time())})) == 0: save_to_json({"price":price_grid, "qty":float(qty), "side":"BUY", "status":"OPEN", "binance_order_id": order_id, "fee": fee_buy, "time": int(time.time())})
    ...
    if side=="SELL":
        qty_db = format_qty(float(order_data['qty'])); res = signed_request("POST", "/api/v3/order", {"symbol":SYMBOL, "side":"SELL", "type":"MARKET", "quantity":qty_db})
        if 'orderId' in res and order_data:
            ...
            profit = (price_grid * qty_fill) - (harga_beli * qty_fill) - fee_buy_db - fee_sell; DAILY_STATS["profit_usdt"] += profit; sb_delete(order_data['id']); usdt, _ = get_all_balance()
...
async def main():
    ...
    global START_TIME, LAST_RECOVERY, PERLU_REENTRY; START_TIME = time.time(); cek_tabel_supabase(); get_binance_rules(SYMBOL)
    ...
    while True:
        try:
            ...
            price = get_price()
            if price > 0: signal_buy, grid_buy = cek_signal_buy(price); signal_sell, grid_sell, order_data, is_top = cek_signal_sell(price);
