...
LOT_USDT = float(os.getenv("LOT", 5)) # [3] LOT 5 USDT FIXED
...
MIN_GRID = 250
MAX_GRID = 1000
QTY_FIXED = 0.00001 # [3] SATPAM 2: QTY FIXED
BUFFER = 0.001
...
async def get_order_params(price): # [3] AMBIL RILL + 2 SATPAM
    try:
        ...
        qty = LOT_USDT / price
        qty = math.floor(qty / lot_step) * lot_step # bulat ke bawah sesuai step
...
def supa_get_positions(): return supa.table("positions").select("*").eq("pair", PAIR).execute().data
def supa_upsert_position(pos): supa.table("positions").upsert(pos, on_conflict="pair,area").execute() # [6]
...
async def place_buy(price):
    ...
    qty, fee, modal_butuh = await get_order_params(price) # [3] Ambil rill saat order + Satpam
    ...
    for _ in range(3): # [2.6]
        try:
            ...
            await send_tele(f"*BUY* `@{price}` | *AREA:* `{area}` | *QTY:* `{qty:.8f}`") # [7]
...
async def check_tp():
    ...
    for pos in supa_get_positions():
        ...
        if price >= tp_price: # [4.4]
            qty, fee, _ = await get_order_params(tp_price) # [4.2] Ambil rill
            try:
...
async def main_loop():
    ...
    while True:
        ...
        except Exception as e: log(traceback.format_exc()); await asyncio.sleep(10)
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE): # [7]
    ...
    pos_text = "\n".join([f"`BUY {p['buy_price']}` -> TP `{p['buy_price']+grid}` AREA `{p['area']}` QTY `{p['qty']:.8f}`" for p in positions]) or "Tidak ada posisi"
