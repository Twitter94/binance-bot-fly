...
NOTIF_MODE = "SILENT"
PAPER_MODE = True # True = Paper, False = Rill
...
def kirim_keyboard():
    keyboard = {"keyboard": [[{"text": "MODE"}, {"text": "STATUS"}], [{"text": "jangan sentuh"}]], "resize_keyboard": True, "one_time_keyboard": False}
    try:
        url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage"
        requests.post(url, data={"chat_id": TELE_CHAT_ID, "text": "✅ <b>Panel Kontrol SPOT Aktif</b>", "parse_mode": "HTML", "reply_markup": json.dumps(keyboard)}, timeout=5)
    except:
        pass
...
def kirim_status_lengkap():
    ...
    mode_notif = "SILENT" if NOTIF_MODE == "SILENT" else "NORMAL"
    mode_uang = "🧪 PAPER" if STATE["paper_mode"] else "💰 BENERAN"
...
{emoji_status} {status_txt} | Notif: {mode_notif} | Uang: {mode_uang}
...
def cek_command_telegram():
    ...
    try:
        ...
        if text == "mode":
            ...
            save_state()
            mode_baru = "PAPER" if STATE["paper_mode"] else "BENERAN SPOT"
...
def get_all_balance():
    if STATE["paper_mode"]:
        return STATE["paper_usdt"], STATE["paper_btc"]
...
def place_order_real(side, price_grid, qty, order_data=None, is_top_grid=False):
    ...
    if side=="BUY":
        ...
        try:
            ...
            if NOTIF_SENT["buy"]!= price_grid:
                notif_penting(f"🟢 <b>BUY TERISI</b>\nHarga: {price_grid:.2f}\nQty: {qty}\nFee: {fee_buy:.4f} USDT\nButuh: {butuh:.2f}\nSaldo USDT: {usdt:.2f}\nJarak: {ATR_MANAGER['jarak']:.2f}")
    ...
    if side=="SELL":
        ...
        if STATE["paper_mode"]:
            ...
            save_state()
            harga_beli = order_data['price']
            profit = (price_grid - harga_beli) * float(qty_db)
            res = {"orderId": f"PAPER_{int(time.time())}", "executedQty": qty_db, "fills": [{"commission": "0", "commissionAsset": "USDT", "price": str(price_grid)}]}
            notif_penting(f"🧪 [PAPER] SELL TP\nHarga: {price_grid:.2f}\nProfit: {profit:.4f} USDT\nSaldo USDT: {STATE['paper_usdt']:.2f}")
        else:
