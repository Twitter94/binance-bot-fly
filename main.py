...
def sync_dan_cek(symbol):
    try:
        ...
        prices_binance_buy = {float(o['price']) for o in bin_orders if o['side'] == 'BUY'}
        prices_binance_sell = {float(o['price']) for o in bin_orders if o['side'] == 'SELL'}
        ...
        for p in prices_binance_buy:
            if p not in prices_supabase: sb_insert(TABEL, {"price": p, "side": "BUY", "status": "active"})
        for p in prices_binance_sell:
            if p not in prices_supabase: sb_insert(TABEL, {"price": p, "side": "SELL", "status": "active"})
        for d in sb_orders:
            p = float(d['price']); side = d['side']
            if side == 'BUY' and p not in prices_binance_buy: sb_delete(TABEL, f"price=eq.{p}&side=eq.BUY")
            if side == 'SELL' and p not in prices_binance_sell: sb_delete(TABEL, f"price=eq.{p}&side=eq.SELL")
    except Exception as e: print(f"[SYNC ERROR] {e}")

# ========== LOGIC BARU: SEMUA DARI BINANCE ==========
def get_filled_buys():
    """FIX 1: AMBIL DARI BINANCE LANGSUNG"""
    try:
...
def is_price_exist(price):
    """FIX 2: CEK DARI BINANCE LANGSUNG"""
    bin_orders = signed_request("GET", "/api/v3/openOrders", {"symbol": SYMBOL})
    for o in bin_orders:
        if abs(float(o['price']) - price) < 0.01 and o['side'] == 'BUY': # toleransi 0.01
            return True
    return False

def cek_total_posisi():
    """FIX 3: CEK BTC DI BINANCE LANGSUNG"""
...
def place_order_real(side, price_grid, qty, is_reentry=False, is_instan_darurat=False):
    global DAILY_STATS
    wajib_sync_dulu()
    if side == "BUY" and is_price_exist(price_grid): print(f"SKIP BUY: Harga {price_grid} sudah ada ordernya"); return None
    ...
    if side == "SELL":
        ...
        # FIX: RE-ENTRY SETELAH SELL NORMAL
...
def cek_sell_instan_darurat(price):
    ...
    # FIX: RE-ENTRY SETELAH SELL INSTAN
...
def cek_signal_sell(price):
    ...
    # Cek ada buy di grid itu dari Binance
    bin_orders = signed_request("GET", "/api/v3/openOrders", {"symbol": SYMBOL})
    ada_buy = any(abs(float(o['price']) - target_grid) < 0.01 and o['side'] == 'BUY' for o in bin_orders)
...
def cek_signal_buy(price):
    ...
    if not FIRST_BUY_DONE:
        ...
        if sisa > 0: print(f"[PEMANASAN] Tunggu {int(sisa)}s lagi"); return None, None
            ...
            if cek_total_posisi() > 0: print("[SYNC] Ternyata udah ada BTC. Skip Buy Paksa"); FIRST_BUY_DONE = True; return None, None
            print(f"[PEMANASAN] Selesai. BUY PAKSA PERTAMA @ {price}")
            qty = hitung_qty_aman(price); fee = get_fee_rate(); butuh_usdt = hitung_butuh_modal(price, qty, fee)
            saldo_usdt, _ = get_all_balance()
            if saldo_usdt >= butuh_usdt: FIRST_BUY_DONE = True; return "BUY", price
...
async def main():
    ...
    auto_create_table(); get_binance_rules(SYMBOL); update_grid_manager(); wajib_sync_dulu()
    saldo_usdt, saldo_btc = get_all_balance(); harga_sekarang = get_price()
    send_telegram(f"🤖 <b>Bot V11.19 FINAL Jalan</b>\nMode: REAL | 100% Binance Raja\n<b>Harga BTC:</b> {harga_sekarang}\n<b>Saldo:</b>\nUSDT: {saldo_usdt:.2f}\nBTC: {saldo_btc:.6f}")
    cek_sell_instan_darurat(harga_sekarang); await asyncio.sleep(3)
    print("Bot V11.19 MODE REAL. Menunggu 10 menit untuk buy pertama...")
