...
def kirim_status():
    harga = get_harga_binance(); saldo = get_binance_balance(); slots = load_slots(); posisi = len(slots); est_profit = 0
    pesan = f"📊 *STATUS v5.39 INFINITE GRID*"
...
def save_env_lot(new_lot):
    global LOT; LOT_LAMA = LOT; LOT = float(new_lot)
    lines = open(".env_rill", "r").readlines(); f = open(".env_rill", "w")
    for line in lines: f.write(f"LOT={new_lot}\n" if line.startswith("LOT=") else line)
    f.close(); return LOT_LAMA
...
def get_atr(force=False):
    ...
    for i in range(3):
        try:
            ...
            atr_baru = sum(tr_list) / len(tr_list); harus_update = False; update_sekarang = False; alasan = ""
            if last_atr > 0:
            ...
            if harus_update:
                ...
                if update_sekarang:
                    ...
                    cancel_all_tp_orders(); slots = load_slots(); new_slots = {buy: float(buy) + new_grid for buy in slots.keys()}
                    save_slots(new_slots); area_yg_aktif.clear(); send_telegram(f"🔄 *TP DIUPDATE SEKARANG*")
                else: send_telegram(f"⚠️ *{alasan}*\n`TP Baru ${new_grid:.2f} Dipakai Pas Reentry`")
                GRID = new_grid; TP = GRID; send_telegram(f"`ATR 14H: ${atr_baru:.2f}`\n`GRID BARU: ${GRID:.2f} | TP: ${TP:.2f}`")
            GRID = max(GRID_MIN, min(atr_baru * ATR_MULTIPLIER, GRID_MAX)); TP = GRID; last_atr = atr_baru; return
        except: time.sleep(2)
        ...
        try:
            ...
            for update in r.get("result", []):
                ...
                if "message" in update and "text" in update["message"]:
                    ...
                    if WAITING_FOR_LOT:
                        try: new_lot = float(text); lot_lama = save_env_lot(new_lot); send_telegram(f"✅ *LOT BERHASIL DIGANTI*\n`LOT Lama: ${lot_lama}`\n`LOT Baru: ${new_lot}`"); WAITING_FOR_LOT = False
...
def place_buy(buy_price):
    ...
    save_slots(slots)
    detail = get_order_details(PAIR, order['orderId']); qty, quote, fee, avg = hitung_dari_fills(detail)
...
def proses_trading():
    ...
    for buy_str, tp_target in list(slots.items()):
        ...
        if harga_sekarang > tp:
            qty = LOT / buy; order = binance_market_sell(qty)
            if 'orderId' in order:
                detail = get_order_details(PAIR, order['orderId']); qty_jual, quote, fee, avg = hitung_dari_fills(detail); profit = quote - LOT - fee
                to_delete_kecepetan.append(buy_str); area_yg_aktif.remove(int(buy / GRID) * GRID)
    ...
    if to_delete_kecepetan:
        ...
        save_slots(slots)
    grid_terdekat = round(harga_sekarang / GRID) * GRID; area_grid = int(grid_terdekat / GRID) * GRID
...
def run_bot():
    ...
    if not slots and RUNNING and harga_awal > 0:
        ...
        if 'orderId' in order:
            ...
            qty, quote, fee, avg_price = hitung_dari_fills(detail)
            buy_price = round(avg_price, 2)
            tp_price = buy_price + TP
            slots[str(buy_price)] = tp_price
            save_slots(slots)
            area_yg_aktif.append(int(buy_price / GRID) * GRID)
    ...
    while True:
        try:
            ...
            if now.hour == ATR_UPDATE_HOUR and now.minute == 0 and now.second < 5: get_atr(force=True); time.sleep(5)
            get_atr(force=False); time.sleep(1)
        except: time.sleep(5)
