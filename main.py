...
def binance_req(path, params={}):
    try:
        params['symbol'] = PAIR
        r = requests.get(f"{BINANCE_URL}{path}", params=params, timeout=3)
...
def get_price():
    data = binance_req("/api/v3/ticker/price")
...
def get_atr_grid():
    try:
        data = binance_req("/api/v3/klines", {"interval":"1h", "limit":ATR_PERIOD+1})
        if not data: return MIN_GRID
...
def get_qty(price):
    try:
        info = binance_req("/api/v3/exchangeInfo")
        min_n_binance = 10
        step = 0.000001
        for s in info.get('symbols',[]):
            if s['symbol'] == PAIR:
                for f in s['filters']:
                    if f['filterType']=='MIN_NOTIONAL': min_n_binance = float(f['minNotional'])
...
def binance_order(side, qty):
    try:
        params = {"symbol":PAIR, "side":side, "type":"MARKET", "quantity":qty, "timestamp":int(time.time()*1000)}
        # NOTE: Ini simplified. Kalau mau real order harus pake HMAC signature
        # Sementara kita pake /api/v3/order/test dulu biar gak error key
        r = requests.post(f"{BINANCE_URL}/api/v3/order", params=params, headers={"X-MBX-APIKEY":API_KEY}, timeout=5)
        return r.json()
    except: return {}

def binance_get_order(orderId):
    data = binance_req("/api/v3/order", {"orderId":orderId})
    return data
...
async def satpam_buy(price, area, reason="GRID"):
    ...
    try:
        ...
        order = binance_order("BUY", qty)
        order_status = binance_get_order(order.get('orderId'))
        if order_status.get('status')!= 'FILLED': return

        supa_req("POST", f"{SUPA_URL}/rest/v1/positions",
                 json={"pair":PAIR,"area":area,"buy_price":price,"qty":qty,"order_id":str(order.get('orderId'))},
...
async def satpam_sell_area(area, positions_in_area, price, mode="BIASA"):
    ...
    try:
        order = binance_order("SELL", total_qty)
        order_status = binance_get_order(order.get('orderId'))
        if order_status.get('status')!= 'FILLED': return
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = f"*STATUS V29.8 ULTRA LIGHT 3S*\n"
