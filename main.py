...
MODE_UANG = "PAPER" # PAPER = simulasi 100 USDT, RILL = uang beneran
SALDO_PAPER = {"usdt": 100.0, "btc": 0.0} # Saldo simulasi
...
def kirim_keyboard():
    keyboard = {"keyboard": [[{"text": "GANTI MODE"}, {"text": "STATUS"}], [{"text": "MODE: PAPER"}, {"text": "MODE: RILL"}]], "resize_keyboard": True, "one_time_keyboard": False}
    try:
        url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage"
        requests.post(url, data={"chat_id": TELE_CHAT_ID, "text": "✅ <b>Panel Kontrol Aktif</b>\n\nKlik PAPER/RILL di bawah cuma buat info\nGanti mode beneran ketik: PAPER atau RILL", "parse_mode": "HTML", "reply_markup": json.dumps(keyboard)}, timeout=5)
    except: 
        pass
...
def cek_command_telegram():
    try:
        ...
        global NOTIF_MODE, MODE_UANG # TAMBAH MODE_UANG
        if text == "ganti mode": 
...
def save_to_json(data):
    try:
        ...
        notif_penting(f"❌ GAGAL SAVE JSON: {repr(e)}")
...
def get_all_balance():
    if MODE_UANG == "PAPER":
        return SALDO_PAPER["usdt"], SALDO_PAPER["btc"]
    ...
    try:
        ...
        for f in data['symbols'][0]['filters']:
            if f['filterType']=='MIN_NOTIONAL': 
                BINANCE_RULES['min_notional']=float(f['minNotional'])
...
def place_order_real(side, price_grid, qty, order_data=None, is_top_grid=False):
    global NOTIF_FLAGS, NOTIF_SENT, BUYING_LOCK, PERLU_REENTRY, LAST_REENTRY_TIME, SALDO_PAPER
    if side=="BUY":
        if price_grid in BUYING_LOCK: 
            return
        if is_price_exist(price_grid) or cek_order_binance_sudah_ada(price_grid): 
            return
        ...
        try: 
            ...
            if usdt < butuh:
                if not NOTIF_FLAGS["saldo_kurang"]: 
                    notif_penting(f"💰 <b>SALDO KURANG</b>\nUSDT: {usdt:.2f} | Butuh: {butuh:.2f}")
            ...
            if MODE_UANG == "PAPER":
                SALDO_PAPER["usdt"] -= nilai_beli
                SALDO_PAPER["btc"] += float(qty)
    ...
    if side=="SELL":
        ...
        if MODE_UANG == "PAPER":
            SALDO_PAPER["btc"] -= float(qty_db)
            SALDO_PAPER["usdt"] += nilai_jual - (nilai_jual * 0.001)
            res = {"orderId": f"PAPER_SELL_{int(time.time())}", "executedQty": qty_db, "fills":[{"commission":"0.001","price":str(price_grid)}]}
        else:
...
async def main():
    ...
    try: 
        ...
        if selisih > 1000:
            notif_penting(f"⚠️ <b>WAKTU VPS MELENCENG {selisih}ms</b>\nOrder bisa gagal. Restart VPS!")
    except: 
        pass
